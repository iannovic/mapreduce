		...
		while (itr.hasMoreTokens()) {
			...
			if (!Global.nodes.isEmpty()) {
				for (int i = 0; i < Global.nodes.size(); i++) {
					Node n2 = Global.nodes.get(i);
					if (n2.getId() == node.getId()) {
						b = true;
						node.setDistance(n2.getDistance());
					}
				} 
			}	
			if (!b) {
			...
			System.out.println("Emitting this from mapper->" + node.toString());				
			...
			//	System.out.println("distance->" + node.toString());	
				context.write(word, node);
			}
		} 
	}
